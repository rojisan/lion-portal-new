...
class ContainerController extends Controller
{
    ...
    {   
        ...
        $response = Http::withHeaders([
            "Accept"=>"application/json",
            "Authorization"=>"Bearer ".session('token'),
        ])->asForm()->get($url . 'cust/get_all_customer?limit=1000');
        $resp = json_decode($response, TRUE);
        if($response->successful()) {
            $custdata = $resp['data'];
            $custdatatArray = [];
    ...
    {   
        ...
        if($searchValue != null || $searchValue != '') {
            $response = Http::withHeaders([
                "Accept"=>"application/json",
                "Authorization"=>"Bearer ".session('token'),
            ])->asForm()->get($url . 'container/search_container_by_no?cont_status=get_in&ct_no=' .$searchValue);
    ...
    {   
        ...
        return DataTables::of($data)
            ->addColumn('action', function($row){
                ...
                data-id="'.$row["ct_id"].'" data-ukuran="'.$row["ct_ukuran"].'" data-container="'.$row["ct_no"].'" data-type="'.$row["ct_type"].'" 
                data-client="'.$row["c_client"].'" data-bm="'.$row["bm_id"].'" data-remark="'.$row["remark_batal_muat"].'">Batal Muat</a>'.$download_btn;
    ...
    {   
        ...
        if($response->successful()) {
            ...
            $response = Http::withHeaders([
                "Accept"=>"application/json",
                "Authorization"=>"Bearer ".session('token'),
            ])->asForm()->get($url . 'acc_bongkar/generate_by_ct_no?ct_no='.$ct_no);
    ...
    {   
        ...
        $response = Http::withHeaders([
            "Accept"=>"application/json",
            "Authorization"=>"Bearer ".session('token'),
        ])->asForm()->get($url . 'depo/get_depo_all?limit=1000');
        $resp = json_decode($response, TRUE);
        if($response->successful()) {
            $depodata = $resp['data'];
            $depodatatArray = [];
        ...
        $response = Http::withHeaders([
            "Accept"=>"application/json",
            "Authorization"=>"Bearer ".session('token'),
        ])->asForm()->get($url . 'trucking/get_trucking_all?limit=1000');
        $resp = json_decode($response, TRUE);
        if($response->successful()) {
            $truckingdata = $resp['data'];
            $truckingdatatArray = [];
        ...
        $response = Http::withHeaders([
            "Accept"=>"application/json",
            "Authorization"=>"Bearer ".session('token'),
        ])->asForm()->get($url . 'barang/get_all_barang?limit=1000');
        $resp = json_decode($response, TRUE);
        if($response->successful()) {
            $jnsdata = $resp['data'];
            $jnsdatatArray = [];
